...
class PurchasesReturnReport extends Component
{
    ...
    {
        $this->validate();
        return Excel::download(new PurchaseReturnExport($this->suppliers, $this->start_date, $this->end_date, $this->supplier_id, $this->purchase_return_status, $this->payment_status),  date('d-M-Y h:i') .' Purchase return.xlsx');
    }
    ...
    {
        return PurchaseReturn::whereDate('date', '>=', $this->start_date)
        ->whereDate('date', '<=', $this->end_date)
        ->when($this->supplier_id, function ($query) {
            return $query->where('supplier_id', $this->supplier_id);
        })
