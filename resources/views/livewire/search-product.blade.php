<div class="position-relative" style="position: fixed !important; width: 100%; height: 100px; z-index: 1000; margin-top: -52px;">
            ...
            <div class="form-group mb-0">
                <div class="input-group">
                    ...
                    <input wire:keydown.escape="resetQuery" wire:model.debounce.500ms="query" type="text"
                        class="form-control" id="search-input" placeholder="Type product name or code...." {{ ($loc) ? '' : 'disabled' }}>
                </div>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        searchInput.addEventListener('keydown', function (event) {
            if (event.key === 'Enter') {
                ...
                if (searchResults) {
                    const firstResult = searchResults.querySelector('li:first-child a');
                    if (firstResult) {
                        firstResult.click();
                    }
                }
