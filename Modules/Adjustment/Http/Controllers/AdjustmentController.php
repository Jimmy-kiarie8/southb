    ...
    public function store(Request $request) {
        ...
        DB::transaction(function () use ($request) {
            ...
            foreach ($request->product_ids as $key => $id) {
                ...
                $product_branch = ProductBranch::where('product_id', $id)->where('branch_id', $request->location_id)->first();

                if ($product_branch) {
                    $product_branch->update([
                        'quantity' => $product_branch->quantity + $qty
                    ]);
                } else {
                    $product_branch = new ProductBranch;
                    $product_branch->quantity($qty, $id, $request->location_id);
                }
            }
        });
